...
export default function ConceptosGenerales() {
  ...
  // ── Constantes isla ──
  const ISLA_KEY             = 4;   // ← Conceptos Generales
  const NIVEL_VISUAL_KEY     = 16;
  const NIVEL_LECTURA_KEY    = 17;
  const NIVEL_RECORDEMOS_KEY = 18;
  const NIVEL_SOCIAL_KEY     = 19;
  const NIVEL_EVALUACION_KEY = 20;  // ← según tabla BD (isla 4 → nivel 20)

  // ── Subislas ──
  const subislas = [
    { id: 1, img: require("../assets/islas/introduccion/visual1.png"),     top: 0.3,  left: 0.04, width: 450, height: 350, screen: "/nivelvisual4" },
    { id: 2, img: require("../assets/islas/introduccion/lectura1.png"),    top: 0.3,  left: 0.23, width: 450, height: 350, screen: "/nivellectura4" },
    { id: 3, img: require("../assets/islas/introduccion/cerebro1.png"),    top: 0.32, left: 0.42, width: 400, height: 300, screen: "/nivelcerebro4" },
    { id: 4, img: require("../assets/islas/introduccion/social1.png"),     top: 0.3,  left: 0.6,  width: 450, height: 350, screen: "/nivelsocial4" },
    { id: 5, img: require("../assets/islas/introduccion/evaluacion1.png"), top: 0.3,  left: 0.8,  width: 450, height: 350, screen: "/evaluacionFinal4" },
  ];
  ...
  const cleanupLegacyOnce = useCallback(async () => {
    ...
    const flag = `u:${usuarioKey}:legacy_cleaned_isla4_v1`;
    ...
    await AsyncStorage.multiRemove([
      ...
      `isla4_nivel${NIVEL_VISUAL_KEY}_visual_done`,
      `isla4_nivel${NIVEL_VISUAL_KEY}_visual_score`,
      `isla4_nivel2_lectura_unlocked`,
      `isla4_nivel${NIVEL_LECTURA_KEY}_lectura_done`,
      `isla4_nivel${NIVEL_LECTURA_KEY}_lectura_score`,
      `isla4_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_done`,
      `isla4_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_score`,
      `isla4_nivel${NIVEL_SOCIAL_KEY}_social_done`,
      `isla4_nivel${NIVEL_SOCIAL_KEY}_social_score`,
      `isla4_nivel${NIVEL_EVALUACION_KEY}_evaluacion_done`,
      `isla4_nivel${NIVEL_EVALUACION_KEY}_evaluacion_score`,
    ]);

    await AsyncStorage.setItem(flag, "true");
    console.log("🧹 Legacy keys isla4 limpiadas para usuario:", usuarioKey);
  }, [usuarioKey]);
  ...
  const loadLocalProgress = useCallback(async () => {
    if (!usuarioKey) return;
    try {
      console.log("📦 Isla4: Loading progress for user:", usuarioKey);
      ...
      // ✅ Evaluación usa NIVEL_EVALUACION_KEY = 20
      ...
      console.log("📦 Isla4: Progreso cargado", { vDone, lDone, rDone, sDone, eDone });
    } catch (e) {
      console.error("📦 Isla4: Error leyendo progreso local:", e);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ Isla4: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 Isla4: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [
    visualDoneLocal,
    lecturaDoneLocal,
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 4, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>CONCEPTOS GENERALES</Text>
            {loadingProgreso ? (
