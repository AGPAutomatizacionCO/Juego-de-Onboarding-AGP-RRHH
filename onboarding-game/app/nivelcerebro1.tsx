/**
 * Nivel Recordemos — Sopa de letras
 */
...
const WORDS: WordInfo[] = [
  ...
  { palabra: "INSPECCION",    concepto: "Revisión técnica para decidir acciones correctivas." },
  { palabra: "NORMATIVIDAD",  concepto: "Conjunto de reglas y estándares técnicos aplicables." },
];
...
// ═════════════════════════════════════════════════════════════════════════════
export default function NivelCerebro() {
  ...
  // ✅ guardamos siempre el origen más reciente
  ...
  // ✅ FIX PISTAS: mide el origen del grid con reintentos si px/py son 0
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: (evt) => {
      // ✅ FIX PISTAS: re-medir SIEMPRE en cada toque para tener
      // el origen actualizado aunque el layout haya cambiado
      medirOrigen();
  ...
  const handleHint = () => {
    ...
    // ✅ FIX PISTAS: limpiar timer anterior, resetear celda y
    // setear la nueva en el siguiente frame garantizado
  ...
  const guardarCompletado = async () => {
    try {
      ...
      await AsyncStorage.multiSet([
        [keyU(`isla1_nivel${NIVEL_KEY}_recordemos_done`),  "true"],
        [keyU(`isla1_nivel${NIVEL_KEY}_recordemos_score`), String(scorePct)],
      ]);
          ...
          <Animated.View style={[st.gameRoot, { opacity: fadeAnim }]}>
            ...
            <View style={st.leftCol}>
              ...
              >
                {cellSize > 0 && (
                  <View
                    ...
                    onLayout={() => {
                      // medir con delay para que el layout nativo ya esté posicionado
                      ...
                      <View key={r} style={st.gridRow}>
                        {row.map((letter, c) => {
                          ...
                          return (
                            <View key={c} style={[
                              st.cell,
                              { width: cellSize, height: cellSize, backgroundColor: bg, borderColor: bd },
                            ]}>
                              <Text style={[
                                st.cellTxt,
                                { fontSize: cellSize * 0.5, color: "#000000", fontWeight: "normal" as const },
                              ]}>
                                {letter}
                              </Text>
                            </View>
              ...
              <View style={st.controls}>
                {/*
                  ✅ FIX SOPA: wordBox tiene altura FIJA (no crece).
                  La palabra seleccionada y el feedback están en la misma
                  fila horizontal, así el box nunca cambia de tamaño
                  y la sopa no se mueve.
                */}
                <View style={[
                  st.wordBox,
                  { borderColor: checkResult === "ok" ? "#16A34A" : checkResult === "fail" ? "#EF4444" : "#3B82F6" },
                ]}>
                  {/* Feedback a la izquierda (o vacío) */}
                  <Text style={[
                    st.feedIcon,
                    {
                ...
                <View style={st.btnRow}>
                  ...
                  <TouchableOpacity style={st.backBtn} onPress={() => router.push("/Introduccion")}>
        ...
        <View style={st.finalOv}>
          <Animated.View style={[
            ...
            <TouchableOpacity
              ...
              disabled={guardando}
              onPress={async () => { await guardarCompletado(); router.push("/Introduccion"); }}
            >
...
const st = StyleSheet.create({
  ...
  introCard: {
    ...
    borderRadius: 25, alignItems: "center", maxWidth: "80%",
  ...
  // ✅ FIX SOPA: altura fija, una sola fila, no crece nunca
  wordBox: {
    borderWidth: 2,
    borderRadius: 10,
    paddingVertical: 6,
    ...
    // altura fija para que nunca cambie el layout
    height: 44,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  conceptTxt:    { fontFamily: "PlusJakartaSans-Regular", fontSize: 13, color: "#1F2937", lineHeight: 17 },
