...
export default function IntroduccionAGP() {
  ...
  // ====== KEYS ======
  const ISLA_KEY           = 2;
  const NIVEL_VISUAL_KEY   = 6;
  const NIVEL_LECTURA_KEY  = 7;
  const NIVEL_RECORDEMOS_KEY = 8;
  const NIVEL_SOCIAL_KEY   = 9;
  const NIVEL_EVALUACION_KEY = 10;
  ...
  const loadLocalProgress = useCallback(async () => {
    ...
    try {
      console.log("📦 HSE: Loading progress for user:", usuarioKey);

      const vDone   = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel1_visual_done`);
      const vScore  = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel1_visual_score`);
      const lUnlock = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel2_lectura_unlocked`);

      setVisualDoneLocal(vDone === "true");
      setVisualScoreLocal(vScore ? Number(vScore) : null);
      setLecturaUnlockedLocal(lUnlock === "true");

      const lDone  = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel2_lectura_done`);
      const lScore = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel2_lectura_score`);
      const rDone  = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel3_recordemos_done`);
      const rScore = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel3_recordemos_score`);
      const sDone  = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel4_social_done`);
      const sScore = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel4_social_score`);
      const eDone  = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel5_evaluacion_done`);
      const eScore = await AsyncStorage.getItem(`u:${usuarioKey}:isla2_nivel5_evaluacion_score`);

      setLecturaDoneLocal(lDone === "true");
      setLecturaScoreLocal(lScore ? Number(lScore) : null);
      setRecordemosDoneLocal(rDone === "true");
      ...
      console.log("📦 HSE: Progreso cargado");
    } catch (e) {
      console.error("📦 HSE: Error leyendo progreso local:", e);
    }
  }, [usuarioKey]);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ HSE: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 HSE: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [visualDoneLocal, lecturaDoneLocal, recordemosDoneLocal, socialDoneLocal, lecturaUnlockedLocal]);
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 2, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>HSE</Text>
            {loadingProgreso ? (
