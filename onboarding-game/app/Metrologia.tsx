...
export default function Metrologia() {
  ...
  // ── Constantes isla ──
  const ISLA_KEY             = 6;
  const NIVEL_VISUAL_KEY     = 26;
  const NIVEL_LECTURA_KEY    = 27;
  const NIVEL_RECORDEMOS_KEY = 28;
  const NIVEL_SOCIAL_KEY     = 29;
  const NIVEL_EVALUACION_KEY = 30;

  // ── Subislas ──
  const subislas = [
    { id: 1, img: require("../assets/islas/introduccion/visual1.png"),     top: 0.3,  left: 0.04, width: 450, height: 350, screen: "/nivelvisual6" },
    { id: 2, img: require("../assets/islas/introduccion/lectura1.png"),    top: 0.3,  left: 0.23, width: 450, height: 350, screen: "/nivellectura6" },
    { id: 3, img: require("../assets/islas/introduccion/cerebro1.png"),    top: 0.32, left: 0.42, width: 400, height: 300, screen: "/nivelcerebro6" },
    { id: 4, img: require("../assets/islas/introduccion/social1.png"),     top: 0.3,  left: 0.6,  width: 450, height: 350, screen: "/nivelsocial6" },
    { id: 5, img: require("../assets/islas/introduccion/evaluacion1.png"), top: 0.3,  left: 0.8,  width: 450, height: 350, screen: "/evaluacionFinal6" },
  ];
  ...
  const cleanupLegacyOnce = useCallback(async () => {
    ...
    const flag = `u:${usuarioKey}:legacy_cleaned_isla8_v1`;
    ...
    await AsyncStorage.multiRemove([
      ...
      `isla8_nivel${NIVEL_VISUAL_KEY}_visual_done`,
      `isla8_nivel${NIVEL_VISUAL_KEY}_visual_score`,
      `isla8_nivel37_lectura_unlocked`,
      `isla8_nivel${NIVEL_LECTURA_KEY}_lectura_done`,
      `isla8_nivel${NIVEL_LECTURA_KEY}_lectura_score`,
      `isla8_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_done`,
      `isla8_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_score`,
      `isla8_nivel${NIVEL_SOCIAL_KEY}_social_done`,
      `isla8_nivel${NIVEL_SOCIAL_KEY}_social_score`,
      `isla8_nivel${NIVEL_EVALUACION_KEY}_evaluacion_done`,
      `isla8_nivel${NIVEL_EVALUACION_KEY}_evaluacion_score`,
    ]);

    await AsyncStorage.setItem(flag, "true");
    console.log("🧹 Legacy keys isla8 limpiadas para usuario:", usuarioKey);
  }, [usuarioKey]);
  ...
  const loadLocalProgress = useCallback(async () => {
    if (!usuarioKey) return;
    try {
      console.log("📦 Isla8: Loading progress for user:", usuarioKey);
      ...
      const lUnlock = await AsyncStorage.getItem(`u:${usuarioKey}:isla${ISLA_KEY}_nivel37_lectura_unlocked`);
      ...
      console.log("📦 Isla8: Progreso cargado", { vDone, lDone, rDone, sDone, eDone });
    } catch (e) {
      console.error("📦 Isla8: Error leyendo progreso local:", e);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ Isla8: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 Isla8: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [
    visualDoneLocal,
    lecturaDoneLocal,
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 6, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>METROLOGÍA</Text>
            {loadingProgreso ? (
