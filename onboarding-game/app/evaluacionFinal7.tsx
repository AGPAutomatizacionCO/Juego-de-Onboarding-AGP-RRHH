...
const RUTA_VOLVER = "/ManipulacionVidrio"; // ⚠️ ajusta al nombre real de tu archivo del mapa
...
const NIVEL_KEY = 25;  // ← Evaluación Final

/* =========================================================
   TIPOS
========================================================= */
type KeyOpt       = "a" | "b" | "c" | "d" | "e" | "f";
type MatrixColumn = string;

type Modulo = { num: number; nombre: string };

type SingleQuestion = { id: number; type: "single"; modulo: Modulo; text: string; options: { key: KeyOpt; label: string }[]; correct: KeyOpt };
type MultiQuestion  = { id: number; type: "multi";  modulo: Modulo; text: string; options: { key: KeyOpt; label: string }[]; correct: KeyOpt[] };
type MatrixRow      = { id: string; label: string; correct: MatrixColumn };
type MatrixQuestion = { id: number; type: "matrix"; modulo: Modulo; text: string; rows: MatrixRow[]; columns: { key: MatrixColumn; label: string }[]; lockRows?: boolean };
...
/* =========================================================
   MÓDULOS
========================================================= */
const MOD1: Modulo = { num: 1, nombre: "Manipulación de blindados" };
const MOD2: Modulo = { num: 2, nombre: "Manipulación de cristales" };
const MOD3: Modulo = { num: 3, nombre: "Manipulación de cristal UMA 1 y 2" };
...
function shuffleMatrix(q: MatrixQuestion): MatrixQuestion {
  // lockRows: las filas mantienen su orden (ej. pasos cronológicos), solo se mezclan las columnas
...
/* =========================================================
   PREGUNTAS — Evaluación Final Manipulación del Vidrio
   24 preguntas en 3 módulos. Las opciones se muestran en
   orden ALEATORIO en cada intento gracias al shuffle.
========================================================= */
const QUESTIONS_BASE: Question[] = [

  /* ═════════ MÓDULO 1 — Manipulación de blindados ═════════ */
  {
    id: 1, type: "single", modulo: MOD1,
    text: "¿Cuál es la única condición para manipular individualmente un vidrio lateral blindado?",
    options: [
      { key: "a", label: "Que pese menos de 25 kg" },
      { key: "b", label: "Que pese menos de 50 kg" }, // ⚠️ en tu lista a y b eran idénticas; cambié esta, corrígela si era otra
      { key: "c", label: "Que sea de blindaje nivel 7" },
      { key: "d", label: "Que se utilicen los EPP" },
    ],
    correct: "a",
  },
  {
    id: 2, type: "single", modulo: MOD1,
    text: "¿En qué condición se debe realizar la manipulación entre 4 personas?",
    options: [
      { key: "a", label: "Cuando la pieza pesa más de 50 kg" },
      { key: "b", label: "Cuando es un vidrio lateral" },
      { key: "c", label: "Cuando es un vidrio blindado nivel 2" },
  ...
  },
  {
    ...
    text: "¿Los EPP solo se utilizan cuando se manipulan vidrios menores al blindaje 5?",
    options: [
      { key: "a", label: "FALSO" },
  ...
  },
  {
    ...
    text: "¿La manipulación de piezas PBS y POS siempre se realiza colectivamente?",
    options: [
  ...
  {
    id: 5, type: "single", modulo: MOD1,
    text: "¿Todas las piezas de blindaje nivel 7 se deben manipular entre 4 personas?",
    options: [
      { key: "a", label: "VERDADERO" },
      { key: "b", label: "FALSO" },
    ],
  ...
  {
    id: 6, type: "single", modulo: MOD1,
    text: "¿Con qué fin se utiliza el antebrazo al manipular el vidrio entre 4 personas?",
    options: [
      { key: "a", label: "Evitar rupturas y accidentes" },
      { key: "b", label: "Evitar dañar el offset y el vidrio paquete de la pieza" },
      { key: "c", label: "Evitar rupturas y daños a los EPP" },
    ],
    correct: "a",
  },
  {
    id: 7, type: "single", modulo: MOD1,
    text: "Al manipular una pieza entre 4 personas... Complete la anterior afirmación",
    options: [
      { key: "a", label: "Debe ubicarse un operario en cada extremo de la pieza" },
      { key: "b", label: "Todos los operarios deben usar los elementos de bioseguridad" },
      { key: "c", label: "Se debe posicionar la pieza con la vista externa hacia arriba" },
      { key: "d", label: "No se pueden manipular piezas entre 4 personas" },
    ],
    correct: "a",
  },

  /* ═════════ MÓDULO 2 — Manipulación de cristales ═════════ */
  {
    id: 8, type: "single", modulo: MOD2,
    text: "¿A quiénes van dirigidas las pautas de manipulación de cristales?",
    options: [
      { key: "a", label: "Al personal administrativo" },
      { key: "b", label: "Al personal operativo de fabricación" },
      { key: "c", label: "Al personal operativo de corte, almacenamiento y despachos" },
      { key: "d", label: "A todo el personal operativo" },
    ],
    correct: "d",
  },
  {
    id: 9, type: "matrix", modulo: MOD2,
    text: "Relacione según corresponda",
    columns: [
      { key: "lamina",   label: "Lámina de vidrio" },
      { key: "paquete",  label: "Vidrio Paquete"   },
      { key: "blindado", label: "Vidrio Blindado"  },
    ],
    rows: [
  ...
  {
    id: 10, type: "multi", modulo: MOD2,
    text: "Seleccione los riesgos del vidrio mencionados en el vídeo",
    options: [
      { key: "a", label: "Rayas" },
      { key: "b", label: "Roturas" },
      { key: "c", label: "Quiñes" },
      { key: "d", label: "Mancha en Banda Negra" },
    ],
    correct: ["a", "b", "c"], // ⚠️ verifica las correctas según el video (asumí Rayas, Roturas y Quiñes)
  },
  {
    id: 11, type: "single", modulo: MOD2,
    text: "Las manillas, anillos y relojes son permitidos siempre y cuando no se utilicen en los procesos de fabricación. La anterior afirmación es:",
    options: [
      { key: "a", label: "VERDADERO" },
      { key: "b", label: "FALSO" },
    ],
  ...
  {
    id: 12, type: "single", modulo: MOD2,
    text: "No hay zonas establecidas para el almacenamiento de vidrios",
    options: [
      { key: "a", label: "FALSO" },
      { key: "b", label: "VERDADERO" },
    ],
  ...
  {
    id: 13, type: "single", modulo: MOD2,
    text: "Los pasillos y áreas de tránsito deben estar despejadas.",
    options: [
      { key: "a", label: "FALSO" },
      { key: "b", label: "VERDADERO" },
    ],
  ...
  {
    id: 14, type: "single", modulo: MOD2,
    text: "Al realizar la manipulación de una pieza pequeña, ¿por qué se descarga la pieza en la pierna?",
    options: [
      { key: "a", label: "Para evitar accidentes" },
      { key: "b", label: "Para evitar que se resbale" },
      { key: "c", label: "Para no rayar el vidrio" },
      { key: "d", label: "Para tener mayor estabilidad" },
    ],
    correct: "b",
  },
  {
    id: 15, type: "single", modulo: MOD2,
    text: "¿Cuántas varillas tienen los carros de piezas grandes?",
    options: [
      { key: "a", label: "2 varillas" },
      { key: "b", label: "3 varillas" },
      { key: "c", label: "5 varillas" },
      { key: "d", label: "4 varillas" },
    ],
    correct: "a",
  },
  {
    id: 16, type: "single", modulo: MOD2,
    text: "¿Cuál es la principal diferencia entre la manipulación de piezas grandes y la manipulación de piezas pequeñas?",
    options: [
      { key: "a", label: "Que se realiza entre dos personas" },
      { key: "b", label: "Que no se descarga la pieza en la pierna" },
      { key: "c", label: "Que debe ubicarse lateral al carro y no en frente" },
      { key: "d", label: "No hay diferencias notables" },
    ],
    correct: "a",
  },
  {
    id: 17, type: "single", modulo: MOD2,
    text: "¿Para qué se aplica alcohol?",
    options: [
      { key: "a", label: "Para que seque más rápido" },
      { key: "b", label: "Para garantizar una mejor limpieza" },
      { key: "c", label: "Para quitar todos los defectos que tiene el vidrio" },
      { key: "d", label: "Para evitar que el vidrio se resbale" },
      { key: "e", label: "Todas las anteriores" },
    ],
    correct: "a",
  },
  {
    id: 18, type: "single", modulo: MOD2,
    text: "¿Cómo debe realizar la inspección del vidrio?",
    options: [
      { key: "a", label: "En Zig-Zag de extremo a extremo" },
      { key: "b", label: "De arriba a abajo, por todos los bordes de la pieza" },
      { key: "c", label: "En contra luz" },
      { key: "d", label: "En Zig-Zag de esquina a esquina" },
    ],
    correct: "a",
  },
  {
    id: 19, type: "single", modulo: MOD2,
    text: "¿Por qué se marcan los defectos en la cara contraria de donde está el defecto?",
    options: [
      { key: "a", label: "Para realizar un bloqueo correcto" },
      { key: "b", label: "Para poder ver mejor el defecto" },
      { key: "c", label: "Para diferenciar los defectos uno del otro" },
    ],
    correct: "a", // ⚠️ no venía marcada la correcta en tu lista; asumí la a, corrígela si era otra
  },
  ...
  {
    id: 21, type: "single", modulo: MOD3,
    text: "¿Qué es lo primero que debe hacer un colaborador al almacenar una pieza pequeña?",
    options: [
      { key: "a", label: "Mano arriba y mano abajo y apoya el vidrio sobre la pierna" },
      { key: "b", label: "Mano arriba y mano al lado y hala el vidrio de forma horizontal hacia usted" },
      { key: "c", label: "Halar el vidrio hacia él, y tomarlo de la parte que le quede más cercana" },
      { key: "d", label: "Ubicar el vidrio por la base y por la parte delantera en la lavadora" },
    ],
    correct: "a",
  },
  {
    id: 22, type: "multi", modulo: MOD3,
    text: "Con base al video, escoja 4 procedimientos en los que se muestra la manipulación de piezas en pareja",
    options: [
      { key: "a", label: "Almacenamiento de pinturas grandes en corte" },
      { key: "b", label: "Montaje y desmontaje de piezas grandes" },
      { key: "c", label: "Manipulación de piezas grandes individualmente" },
      { key: "d", label: "Manipulación de piezas grandes en pareja" },
      { key: "e", label: "Vidrios PBS o piezas grandes de serigrafía" },
      { key: "f", label: "Trasporte hacia el caballete donde se posicionan logos" },
    ],
    correct: ["a", "b", "d", "e"], // ⚠️ verifica los 4 correctos según el video (asumí todos menos "individualmente" y "caballete")
  },
  {
    id: 23, type: "single", modulo: MOD3,
    text: "En serigrafía, todos los vidrios PBS o piezas grandes deben ser manipulados siempre entre dos personas.",
    options: [
      { key: "a", label: "FALSO" },
      { key: "b", label: "VERDADERO" },
    ],
  ...
  {
    id: 24, type: "matrix", modulo: MOD3, lockRows: true,
    text: "Identifique el procedimiento a realizar y organice en el orden cronológico correspondiente",
    columns: [
      { key: "hala",    label: "Hala el vidrio" },
      { key: "inclina", label: "Inclina el vidrio, y lo toma una mano arriba y una mano abajo" }, // ⚠️ verifica el texto completo (salía cortado en la imagen)
      { key: "alza",    label: "Alza la pieza manteniendo la posición una mano arriba y otra abajo" },  // ⚠️ verifica el texto completo (salía cortado en la imagen)
      { key: "ubica",   label: "Ubica la pieza en el destino" }, // ⚠️ verifica el texto completo (salía cortado en la imagen)
    ],
    rows: [
      { id: "f1", label: "PRIMERO", correct: "hala"    }, // ⚠️ verifica el orden cronológico correcto
      { id: "f2", label: "SEGUNDO", correct: "inclina" },
      { id: "f3", label: "TERCERO", correct: "alza"    },
      { id: "f4", label: "CUARTO", correct: "ubica"   },
    ],
  },
];
...
const MATRIX_LABEL_W  = scaleDP(160);
const MATRIX_COL_W    = scaleDP(100);
const MATRIX_ROW_H    = scaleDP(44);
const MATRIX_FONT     = scaleDP(12);
const MATRIX_HEADER_F = scaleDP(11);
...
========================================================= */
export default function NivelEvaluacionFinalManipulacion() {
    ...
    (async () => {
      ...
      if (finalUk) {
        ...
        if (evaDone === "true") {
          ...
          setScore(pct); setShowIntro(false); setShowResult(true);
        }
        try {
  ...
  /* ── Reset ── */
  const resetAll = () => {
    setQIndex(0); setAnswersSingle({}); setAnswersMulti({}); setAnswersMatrix({});
        ...
        <View style={st.header}>
          <View style={st.introBox}>
            ...
            <Text style={st.descripcionIntro}>
              Llegaste al cierre del recorrido de Manipulación del Vidrio.{"\n\n"}
              La evaluación tiene 3 módulos: Manipulación de blindados, Manipulación de cristales y Manipulación de cristal UMA 1 y 2.{"\n\n"}
        ...
        <View style={st.overlay}>
          <View style={st.podioBox}>
            <View style={st.podioHeader}>
              <Text style={st.podioTitle}>🏆 Podio Manipulación del Vidrio</Text>
            ...
            <View style={{ flexDirection: "row", gap: scaleDP(10), marginTop: scaleDP(14) }}>
              <TouchableOpacity style={[st.podioBtn, { backgroundColor: "#a3ecf1", flex: 1 }]}
