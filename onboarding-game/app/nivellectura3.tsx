...
const ISLA_KEY = 3;
const NIVEL_KEY_API = 12;
const NIVEL_KEY_PROG = 2;
const MAX_LIVES = 3;
...
const preguntasDefault = [
  { id: 1, texto: "Cuando a los vidrios se les realiza un desbaste para eliminar bordes cortantes y mejorar la estética, corresponde al proceso de", respuesta: "Mecanizado" as OpcionRespuesta },
  { id: 2, texto: "Cuando el vidrio pasa por calor para fijar la pintura correctamente, corresponde al proceso de", respuesta: "Vitrificado" as OpcionRespuesta },
  { id: 3, texto: "Cuando al vidrio se le realiza un baño de sales para hacerlo más resistente, corresponde al proceso de", respuesta: "Templado Químico" as OpcionRespuesta },
  { id: 4, texto: "Cuando se aplica talco para evitar que los vidrios se adhieran entre sí, corresponde al proceso de", respuesta: "Empalme" as OpcionRespuesta },
  { id: 5, texto: "Cuando se inspecciona y verifica el vidrio blindado cumpliendo normativas, corresponde al proceso de", respuesta: "Control Final" as OpcionRespuesta },
  { id: 6, texto: "Cuando los vidrios y plásticos se compactan con presión y temperatura, corresponde al proceso de", respuesta: "Autoclave" as OpcionRespuesta },
];
...
export default function NivelLecturaProcesos() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (uk && uk > 0) {
        ...
        const doneKey  = `u:${uk}:isla${ISLA_KEY}_nivel${NIVEL_KEY_PROG}_lectura_done`;
        const scoreKey = `u:${uk}:isla${ISLA_KEY}_nivel${NIVEL_KEY_PROG}_lectura_score`;
  ...
  const [opcionesRespuesta,  setOpcionesRespuesta]  = useState<string[]>(opcionesRespuestaDefault);
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const resp = await fetch(`${API_URL}/api/niveles/lectura/${NIVEL_KEY_API}`);
        const data = await resp.json();
        const items: any[] = data?.data || data || [];

        if (Array.isArray(items) && items.length > 0) {
          const pgs = items.map((item: any, idx: number) => ({
            id: idx + 1,
            texto:     item.LECTURA_TEXTO     || item.texto     || item.pregunta || "",
            respuesta: item.LECTURA_RESPUESTA || item.respuesta || item.correcta || "",
          }));
          setPreguntas(pgs);
          const opts = [...new Set(pgs.map((p) => p.respuesta))] as string[];
          setOpcionesRespuesta(opts);
          generarOpcionesAleatorias(pgs, opts);

          const initResp: Record<number, string | null> = {};
          const initFeed: EstadoFeedback = {};
          pgs.forEach((p) => { initResp[p.id] = null; initFeed[p.id] = null; });
          setRespuestas(initResp);
          setFeedback(initFeed);
        }
      } catch (e) {
        console.log("Usando preguntas por defecto:", e);
      }
    };
    loadQuestions();
  }, []);
  ...
  const guardarProgreso = async (score: number) => {
    try {
      ...
      await AsyncStorage.multiSet([
        [`u:${uk}:isla${ISLA_KEY}_nivel${NIVEL_KEY_PROG}_lectura_done`,  "true"],
        [`u:${uk}:isla${ISLA_KEY}_nivel${NIVEL_KEY_PROG}_lectura_score`, String(score)],
        [`u:${uk}:isla${ISLA_KEY}_nivel3_recordemos_unlocked`,            "true"],
      ]);
      ...
      console.error("Error guardando progreso lectura 3:", e);
    }
  };
      ...
      {alreadyPlayed && savedScore !== null && (
        <View style={styles.header}>
          <View style={styles.introBox}>
            <Text style={styles.titulo}>Nivel Lectura – Procesos de Producción</Text>
        ...
        <View style={styles.header}>
          <View style={styles.introBox}>
            ...
            <Text style={styles.descripcion}>
              En este nivel deberás leer atentamente cada definición y seleccionar el proceso correcto.{"\n\n"}
