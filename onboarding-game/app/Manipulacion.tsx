...
export default function ManipulacionVidrio() {
  ...
  const NIVEL_VISUAL_KEY     = 21;
  const NIVEL_LECTURA_KEY    = 22;
  const NIVEL_RECORDEMOS_KEY = 23;
  const NIVEL_SOCIAL_KEY     = 24;
  const NIVEL_EVALUACION_KEY = 25;

  // ── Subislas ──
  // ⚠️ Ajusta los "screen" a los nombres reales de tus archivos de niveles de Manipulación
  const subislas = [
    { id: 1, img: require("../assets/islas/introduccion/visual1.png"),     top: 0.3,  left: 0.04, width: 450, height: 350, screen: "/nivelvisual7" },
    { id: 2, img: require("../assets/islas/introduccion/lectura1.png"),    top: 0.3,  left: 0.23, width: 450, height: 350, screen: "/nivellectura7" },
    { id: 3, img: require("../assets/islas/introduccion/cerebro1.png"),    top: 0.32, left: 0.42, width: 400, height: 300, screen: "/nivelcerebro7" },
    { id: 4, img: require("../assets/islas/introduccion/social1.png"),     top: 0.3,  left: 0.6,  width: 450, height: 350, screen: "/nivelsocial7" },
    { id: 5, img: require("../assets/islas/introduccion/evaluacion1.png"), top: 0.3,  left: 0.8,  width: 450, height: 350, screen: "/evaluacionFinal7" },
  ];
  ...
  const cleanupLegacyOnce = useCallback(async () => {
    ...
    const flag = `u:${usuarioKey}:legacy_cleaned_isla5_v1`;
    ...
    await AsyncStorage.multiRemove([
      ...
      `isla5_nivel${NIVEL_VISUAL_KEY}_visual_done`,
      `isla5_nivel${NIVEL_VISUAL_KEY}_visual_score`,
      `isla5_nivel22_lectura_unlocked`,
      `isla5_nivel${NIVEL_LECTURA_KEY}_lectura_done`,
      `isla5_nivel${NIVEL_LECTURA_KEY}_lectura_score`,
      `isla5_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_done`,
      `isla5_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_score`,
      `isla5_nivel${NIVEL_SOCIAL_KEY}_social_done`,
      `isla5_nivel${NIVEL_SOCIAL_KEY}_social_score`,
      `isla5_nivel${NIVEL_EVALUACION_KEY}_evaluacion_done`,
      `isla5_nivel${NIVEL_EVALUACION_KEY}_evaluacion_score`,
    ]);

    await AsyncStorage.setItem(flag, "true");
    console.log("🧹 Legacy keys isla5 limpiadas para usuario:", usuarioKey);
  }, [usuarioKey]);
  ...
  const loadLocalProgress = useCallback(async () => {
    if (!usuarioKey) return;
    try {
      console.log("📦 Isla5: Loading progress for user:", usuarioKey);
      ...
      const lUnlock = await AsyncStorage.getItem(`u:${usuarioKey}:isla${ISLA_KEY}_nivel22_lectura_unlocked`);
      ...
      console.log("📦 Isla5: Progreso cargado", { vDone, lDone, rDone, sDone, eDone });
    } catch (e) {
      console.error("📦 Isla5: Error leyendo progreso local:", e);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ Isla5: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 Isla5: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [
    visualDoneLocal,
    lecturaDoneLocal,
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 5, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>MANIPULACIÓN DEL VIDRIO</Text>
            {loadingProgreso ? (
