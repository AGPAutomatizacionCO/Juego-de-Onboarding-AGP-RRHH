...
/* =========================================================
   CONTENIDO FIJO — Manipulación del Vidrio (Isla 5)
========================================================= */
const FRASES_MANIPULACION: Frase[] = [
  {
    id: 1,
    antes: "Al realizar la manipulación de una pieza pequeña, esta se descarga sobre la",
    respuesta: "Pierna",
    despues: "para evitar que se resbale.",
  },
  {
    id: 2,
    antes: "Los carros de almacenamiento para piezas grandes cuentan con",
    respuesta: "2 varillas",
    despues: ".",
  },
  {
    id: 3,
    antes: "Los carros de almacenamiento para piezas pequeñas cuentan con",
    respuesta: "4 varillas",
    despues: ".",
  },
  {
    id: 4,
    antes: "El primer paso para realizar la limpieza de una pieza es aplicar",
    respuesta: "Alcohol",
    despues: "sobre la superficie.",
  },
  {
    id: 5,
    antes: "El alcohol se utiliza para que la pieza",
    respuesta: "Seque",
    despues: "más rápido durante el proceso de limpieza.",
  },
  {
    id: 6,
    antes: "Durante la inspección, el vidrio debe revisarse en forma de",
    respuesta: "Zig-Zag",
    despues: "de extremo a extremo.",
  },
  {
    id: 7,
    antes: "Cuando el vidrio pesa más de",
    respuesta: "50 kg",
    despues: "la manipulación debe realizarse con 4 personas.",
  },
];

const OPCIONES_INCORRECTAS: string[] = [
  "Brazo",
  "3 varillas",
  "Agua",
  "Horizontal",
...
const OPCIONES_MANIPULACION: string[] = [
  ...FRASES_MANIPULACION.map((f) => f.respuesta),
...
/* ---------- Pantalla principal ---------- */
export default function NivelLecturaManipulacion() {
  const router = useRouter();

  const API_URL = API_BASE_URL;

  // ── Anclaje isla Manipulación del Vidrio ──
  const ISLA_KEY  = 5;
  const NIVEL_KEY = 22;
  ...
  const PROG_RECORDEMOS_UNLOCK_KEY = keyU(`isla${ISLA_KEY}_nivel23_recordemos_unlocked`);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setFrases(FRASES_MANIPULACION);
        setOpciones(shuffle(OPCIONES_MANIPULACION));
        setShowIntro(true);
      } catch (e: any) {
        ...
        {showIntro && !showContent && (
          <View style={styles.header}>
            <View style={styles.introBox}>
              <Text style={styles.tituloIntro}>Nivel de Lectura – Manipulación del Vidrio</Text>
              <Text style={styles.descripcionIntro}>
                En este nivel pondrás a prueba lo aprendido sobre la manipulación segura del vidrio en AGP.{"\n\n"}
                Lee con atención cada frase y arrastra la palabra correcta al espacio vacío
                para completar el significado.{"\n\n"}
                Recuerda las técnicas de descarga, los carros de almacenamiento, la limpieza
                y la inspección de las piezas.
              </Text>
              <TouchableOpacity
          ...
          <View style={styles.modalOverlay}>
            ...
            >
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setSuccessVisible(false);
                  router.replace(RUTA_VOLVER as any);
                }}
              >
