...
const API_ISLAS    = `${API_BASE_URL}/api/islas`;
const ISLA_KEY  = 2;
const NIVEL_KEY = 10;
...
const QUESTIONS_FALLBACK: Question[] = [
  {
    id: 1,
    text: "¿Por qué es importante etiquetar correctamente los envases de sustancias químicas según el SGA?",
    options: [
      { key: "a", label: "Para cumplir solo con un requisito administrativo" },
      { key: "b", label: "Para identificar el proveedor del producto" },
      { key: "c", label: "Para comunicar claramente los peligros y medidas de seguridad del producto" },
      { key: "d", label: "Para mejorar la apariencia del envase" },
    ],
    correct: "c",
  },
  {
    id: 2,
    text: "¿Qué significa el pictograma del SGA con una llama?",
    options: [
      { key: "a", label: "Sustancias corrosivas" },
      { key: "b", label: "Sustancias inflamables" },
      { key: "c", label: "Sustancias tóxicas" },
      { key: "d", label: "Sustancias explosivas" },
    ],
    correct: "b",
  },
  {
    id: 3,
    text: "¿Qué es un acto inseguro y una condición insegura?",
    options: [
      { key: "a", label: "Ambos dependen únicamente del trabajador" },
      { key: "b", label: "El acto inseguro es una acción incorrecta y la condición insegura es una situación peligrosa del entorno" },
      { key: "c", label: "El acto inseguro es una falla del equipo y la condición insegura es un error humano" },
      { key: "d", label: "Ambos se refieren solo a fallas técnicas" },
    ],
    correct: "b",
  },
  {
    id: 4,
    text: "¿Cuál de los siguientes es un ejemplo de acto inseguro en el lugar de trabajo?",
    options: [
      { key: "a", label: "Piso mojado sin señalización" },
      { key: "b", label: "Máquina sin guardas de seguridad" },
      { key: "c", label: "No usar los elementos de protección personal" },
      { key: "d", label: "Iluminación deficiente" },
    ],
    correct: "c",
  },
  {
    id: 5,
    text: "¿Qué es el riesgo mecánico?",
    options: [
      { key: "a", label: "La probabilidad de incendios por sustancias químicas" },
      { key: "b", label: "La posibilidad de sufrir daños por la interacción con máquinas, equipos o herramientas" },
      { key: "c", label: "El riesgo asociado a posturas inadecuadas" },
      { key: "d", label: "La exposición a agentes biológicos" },
    ],
    correct: "b",
  },
  {
    id: 6,
    text: "¿Cuál de las siguientes opciones incluye medidas preventivas para el manejo de máquinas, equipos y herramientas?",
    options: [
      { key: "a", label: "Trabajar con prisa, retirar guardas y no usar EPP" },
      { key: "b", label: "Usar EPP adecuado, seguir procedimientos y realizar mantenimiento preventivo" },
      { key: "c", label: "Improvisar herramientas y operar equipos sin capacitación" },
      { key: "d", label: "Ignorar señalización de seguridad" },
    ],
    correct: "b",
  },
];
...
export default function NivelEvaluacionFinalHSE() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (finalUk) {
        const evaDone  = await AsyncStorage.getItem(`u:${finalUk}:isla2_nivel5_evaluacion_done`);
        const evaScore = await AsyncStorage.getItem(`u:${finalUk}:isla2_nivel5_evaluacion_score`);
        if (evaDone === "true") {
  ...
  const saveScoreToDB = async (pct: number) => {
    ...
    await AsyncStorage.multiSet([
      [`u:${usuarioKey}:isla2_nivel5_evaluacion_done`,  "true"],
      [`u:${usuarioKey}:isla2_nivel5_evaluacion_score`, String(pct)],
    ]);
    try {
      await fetch(`${API_ISLAS}/${usuarioKey}/avanzar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nuevaIsla: 9 }),
      });
    } catch (e) { console.log("Error desbloqueando islas:", e); }
    setSavedPct(pct);
  };
  ...
  const loadPodio = async () => {
    ...
    try {
      ...
      Alert.alert("Error al cargar podio", e?.message || "No se pudo cargar."); setPlayers([]);
        ...
        <View style={styles.header}>
          <View style={styles.introBox}>
            ...
            <Text style={styles.descripcionIntro}>
              Llegaste al cierre del recorrido de HSE.{"\n"}
              Responde las preguntas sobre seguridad, pictogramas, riesgos, actos inseguros y medidas preventivas.{"\n\n"}
              Puedes moverte entre las preguntas con los botones de navegación, revisar y ajustar tus respuestas antes de finalizar.
            </Text>
        ...
        <View style={styles.overlay}>
          <View style={styles.resultAlertBox}>
            ...
            <Text style={styles.resultMainText}>
              {passed ? "¡Excelente! Has aprobado la evaluación final 🎉"
            ...
            <View style={styles.resultRow}>
              ...
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#4C92E4", marginTop: scaleDP(18) }]} onPress={() => router.push(RUTA_VOLVER as any)}>
            ...
            >
              <Text style={[styles.modalBtnText, { color: "#78350F" }]}>{podioLoading ? "Cargando podio..." : "Ver podio 🏆"}</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
        ...
        <View style={styles.overlay}>
          <View style={styles.podioBox}>
            <View style={styles.podioHeader}>
              <Text style={styles.podioTitle}>🏆 Podio HSE</Text>
            ...
            <View style={{ flexDirection: "row", gap: scaleDP(10), marginTop: scaleDP(14) }}>
              <TouchableOpacity style={[styles.podioBtn, { backgroundColor: "#a3ecf1", flex: 1 }]} onPress={() => { setShowPodio(false); setShowResult(true); }}>
                <Text style={styles.podioBtnText}>Volver</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.podioBtn, { backgroundColor: "#a3ecf1", flex: 1 }]} onPress={() => router.push("/mapa" as any)}>
