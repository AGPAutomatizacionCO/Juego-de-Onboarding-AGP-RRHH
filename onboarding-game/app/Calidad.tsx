...
export default function Calidad() {
  ...
  // ── Constantes isla ──
  const ISLA_KEY             = 8;   // ← Calidad
  const NIVEL_VISUAL_KEY     = 36;
  const NIVEL_LECTURA_KEY    = 37;
  const NIVEL_RECORDEMOS_KEY = 38;
  const NIVEL_SOCIAL_KEY     = 39;
  const NIVEL_EVALUACION_KEY = 40;

  // ── Subislas ──
  const subislas = [
    { id: 1, img: require("../assets/islas/introduccion/visual1.png"),     top: 0.3,  left: 0.04, width: 450, height: 350, screen: "/nivelvisual5" },
    { id: 2, img: require("../assets/islas/introduccion/lectura1.png"),    top: 0.3,  left: 0.23, width: 450, height: 350, screen: "/nivellectura5" },
    { id: 3, img: require("../assets/islas/introduccion/cerebro1.png"),    top: 0.32, left: 0.42, width: 400, height: 300, screen: "/nivelcerebro5" },
    { id: 4, img: require("../assets/islas/introduccion/social1.png"),     top: 0.3,  left: 0.6,  width: 450, height: 350, screen: "/nivelsocial5" },
    { id: 5, img: require("../assets/islas/introduccion/evaluacion1.png"), top: 0.3,  left: 0.8,  width: 450, height: 350, screen: "/evaluacionFinal5" },
  ];
  ...
  const cleanupLegacyOnce = useCallback(async () => {
    ...
    const flag = `u:${usuarioKey}:legacy_cleaned_isla7_v1`;
    ...
    console.log("🧹 Legacy keys isla7 limpiadas para usuario:", usuarioKey);
  }, [usuarioKey]);
  ...
  const loadLocalProgress = useCallback(async () => {
    if (!usuarioKey) return;
    try {
      console.log("📦 Isla7: Loading progress for user:", usuarioKey);
      ...
      console.log("📦 Isla7: Progreso cargado", { vDone, lDone, rDone, sDone, eDone });
    } catch (e) {
      console.error("📦 Isla7: Error leyendo progreso local:", e);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ Isla7: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 Isla7: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [
    visualDoneLocal,
    lecturaDoneLocal,
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 8, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>CALIDAD</Text>
            {loadingProgreso ? (
