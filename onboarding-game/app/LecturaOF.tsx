...
export default function LecturaOF() {
  ...
  // ── Constantes isla ──
  const ISLA_KEY             = 7;   // ← Lectura OF (según tabla BD Onboarding_Islas)
  const NIVEL_VISUAL_KEY     = 31;
  const NIVEL_LECTURA_KEY    = 32;
  const NIVEL_RECORDEMOS_KEY = 33;
  const NIVEL_SOCIAL_KEY     = 34;
  const NIVEL_EVALUACION_KEY = 35;

  // ── Subislas ──
  // ⚠️ Ajusta los "screen" a los nombres reales de tus archivos de niveles de Lectura OF
  const subislas = [
    { id: 1, img: require("../assets/islas/introduccion/visual1.png"),     top: 0.3,  left: 0.04, width: 450, height: 350, screen: "/nivelvisual8" },
    { id: 2, img: require("../assets/islas/introduccion/lectura1.png"),    top: 0.3,  left: 0.23, width: 450, height: 350, screen: "/nivellectura8" },
    { id: 3, img: require("../assets/islas/introduccion/cerebro1.png"),    top: 0.32, left: 0.42, width: 400, height: 300, screen: "/nivelcerebro8" },
    { id: 4, img: require("../assets/islas/introduccion/social1.png"),     top: 0.3,  left: 0.6,  width: 450, height: 350, screen: "/nivelsocial8" },
    { id: 5, img: require("../assets/islas/introduccion/evaluacion1.png"), top: 0.3,  left: 0.8,  width: 450, height: 350, screen: "/evaluacionFinal8" },
  ];
  ...
  const cleanupLegacyOnce = useCallback(async () => {
    ...
    const flag = `u:${usuarioKey}:legacy_cleaned_isla7_lecturaof_v1`;
    ...
    await AsyncStorage.multiRemove([
      ...
      `isla7_nivel${NIVEL_VISUAL_KEY}_visual_done`,
      `isla7_nivel${NIVEL_VISUAL_KEY}_visual_score`,
      `isla7_nivel32_lectura_unlocked`,
      `isla7_nivel${NIVEL_LECTURA_KEY}_lectura_done`,
      `isla7_nivel${NIVEL_LECTURA_KEY}_lectura_score`,
      `isla7_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_done`,
      `isla7_nivel${NIVEL_RECORDEMOS_KEY}_recordemos_score`,
      `isla7_nivel${NIVEL_SOCIAL_KEY}_social_done`,
      `isla7_nivel${NIVEL_SOCIAL_KEY}_social_score`,
      `isla7_nivel${NIVEL_EVALUACION_KEY}_evaluacion_done`,
      `isla7_nivel${NIVEL_EVALUACION_KEY}_evaluacion_score`,
    ]);

    await AsyncStorage.setItem(flag, "true");
    console.log("🧹 Legacy keys isla7 (Lectura OF) limpiadas para usuario:", usuarioKey);
  }, [usuarioKey]);
  ...
  const loadLocalProgress = useCallback(async () => {
    if (!usuarioKey) return;
    try {
      console.log("📦 Isla7 LecturaOF: Loading progress for user:", usuarioKey);
      ...
      const lUnlock = await AsyncStorage.getItem(`u:${usuarioKey}:isla${ISLA_KEY}_nivel32_lectura_unlocked`);
      ...
      console.log("📦 Isla7 LecturaOF: Progreso cargado", { vDone, lDone, rDone, sDone, eDone });
    } catch (e) {
      console.error("📦 Isla7 LecturaOF: Error leyendo progreso local:", e);
  ...
  const loadBDProgress = useCallback(async () => {
    ...
    try {
      ...
      console.error("⚠️ Isla7 LecturaOF: Error cargando progreso:", e?.message || e);
    } finally {
  ...
  const progresoNivelEfectivo = useMemo(() => {
    ...
    console.log("🎯 Isla7 LecturaOF: Progreso efectivo:", { localByDone, localUnlock, effective });
    return effective;
  }, [
    visualDoneLocal,
    lecturaDoneLocal,
  ...
  const onPressNivel = async (nivelId: number, screen: string) => {
    ...
    if (alreadyDone) {
      const reintentoOk = await intentarConsumirReintento(usuarioKey, 7, nivelId);
      if (!reintentoOk) {
    ...
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={styles.titulo}>LECTURA DE ORDEN DE FABRICACIÓN</Text>
            {loadingProgreso ? (
